      ...
      <main style={{ flex: 1, overflow: "auto", display: "flex", flexDirection: "column", paddingBottom: isMobile ? 72 : 0 }}>
        ...
        {isMobile ? (
          <header style={{ padding: "10px 16px", borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", background: T.bg1, position: "sticky", top: 0, zIndex: 10, minHeight: 48 }}>
            ...
            <button onClick={() => setSettingsOpen(true)} style={{ background: "transparent", border: "none", cursor: "pointer", color: T.textDim, padding: 8, minWidth: 44, minHeight: 44, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 8, transition: "all .15s", position: "relative" }}
              onMouseEnter={e => { e.currentTarget.style.color = T.accent; e.currentTarget.style.background = T.bg3; }}
              onMouseLeave={e => { e.currentTarget.style.color = T.textDim; e.currentTarget.style.background = "transparent"; }}>
              ...
              {bugCount > 0 && <span style={{ position: "absolute", top: 6, right: 6, width: 8, height: 8, borderRadius: "50%", background: T.danger }} />}
            </button>
          </header>
        ) : (
      ...
      {isMobile && settingsOpen && (
        <div onClick={() => setSettingsOpen(false)} style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", zIndex: 30, display: "flex", justifyContent: "flex-end" }}>
          <div onClick={e => e.stopPropagation()} style={{ width: 280, maxWidth: "80vw", height: "100%", background: T.bg1, borderLeft: `1px solid ${T.border}`, padding: 24, display: "flex", flexDirection: "column", gap: 8, transform: "translateX(0)", transition: "transform .25s ease" }}>
            ...
            {/* Report Bug */}
            <button onClick={() => { setBugReportOpen(true); setSettingsOpen(false); }} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 0", border: "none", background: "transparent", cursor: "pointer", color: T.text, fontSize: 14, minHeight: 44, width: "100%", textAlign: "left", fontFamily: "inherit" }}>
              <BugIcon size={18} />
              <span>Report Bug</span>
              {bugCount > 0 && (
                <span style={{ marginLeft: "auto", fontSize: 9, fontWeight: 800, background: T.danger, color: "#fff", minWidth: 16, height: 16, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 4px" }}>
                  {bugCount}
                </span>
              )}
            </button>
